...
export const startPgDriverSuite = async () => {
  ...
  suite
    ...
    .add('Pg Driver Suppliers: getAll', async () => {
      await db.query('select * from "suppliers"');
    })
    .add('Pg Driver Suppliers: getInfo', async () => {
      await db.query('select * from "suppliers" where "suppliers"."id" = $1', ['1']);
    })

    .add('Pg Driver Products: getAll', async () => {
      await db.query('select * from "products"');
    })
    .add('Pg Driver Products: getInfo', async () => {
    ...
    .add('Pg Driver Products: search', async () => {
      await db.query('select * from "products" where "products"."name" ilike $1', ['cha']);
    })

    .add('Pg Driver Orders: getAll', async () => {
    ...
    .add('Pg Driver Orders: getInfo', async () => {
